...
def generate_camera_data(camera_id):
    # Adjust in/out counts based on the camera ID
    if camera_id == "clemons_camera_6":
        in_count = 30
        out_count = 0  # Assuming no "out" events for camera 6
    elif camera_id == "clemons_camera_5":
        in_count = 4
        out_count = 2
    else:  # Default case for cameras 1, 2, 3, and 4
        in_count = 10
        out_count = 2
...
def upload_camera_data_to_s3(camera_id, data):
    # Store each camera's data under its own prefix
